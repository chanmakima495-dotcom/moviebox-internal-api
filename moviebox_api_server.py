...
def get_session(session_id: Optional[str] = None):
    ...
    try:
        ...
        HARDCODED = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1aWQiOjcwNjU5NDg0MTAyMTM4MTYyMzIsInV0cCI6MSwiZXhwIjoxNzkxNzMyMjMzLCJpYXQiOjE3ODM5NTU5MzN9.7iyEzTj4vWAbOF0oXwNnZ0p3Nc1QaO6K9eMiGFyVfGs"
        if auth.token and auth.token != HARDCODED:
            logger.info(f"Bootstrap guest token OK: {auth.token[:30]}... UID: {auth.user_id}")
            auth.is_logged_in = True
        else:
            logger.warning("Bootstrap guest token did not update.")
    except Exception as e:
...
# Ã¢â€â‚¬Ã¢â€â‚¬ Models Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
def map_item(src: dict, depth: int = 0):
    ...
    title = (
        ...
        item.get("show_name") or item.get("showTitle") or item.get("titleName") or
        item.get("title_en") or item.get("tag") or item.get("label") or
        item.get("extra") or item.get("subtitle") or item.get("tabName") or
        item.get("tab_name") or item.get("searchName") or item.get("promotionName") or
        src.get("title") or src.get("name") or src.get("content") or
        src.get("keyword") or src.get("label") or "Unknown"
    )
    ...
    if title == "Unknown" and action_type == "category" and category_id:
    ...
    if not poster_url:
        ...
        poster_url = cover.get("url") if isinstance(cover, dict) else (cover or "")
    ...
    if not poster_url:
        ...
        poster_url = hp.get("url") if isinstance(hp, dict) else (hp or "")

    if not poster_url:
        banner = item.get("banner")
        if isinstance(banner, dict):
            poster_url = banner.get("image", {}).get("url") or banner.get("url") or ""
    ...
    return {
        ...
        "likeStatus": 1 if (
            item.get("isFavorite") == 1 or item.get("is_favorite") == 1 or
            item.get("fav") == 1 or item.get("is_fav") == 1 or
            item.get("isLike") == 1 or item.get("is_like") == 1 or
            item.get("wantToSee") == 1 or item.get("likeStatus") == 1 or
            item.get("collected") == 1 or item.get("isCollect") == 1 or
            item.get("collectedStatus") == 1 or str(item.get("likeType")) == "1" or
            item.get("isCollect") is True or item.get("isFavorite") is True
        ) else 0,
...
def format_tab_sections(items: list):
    ...
    if is_direct_movies and items:
        ...
        if mapped:
            return [{"title": "Content", "items": mapped}]
    ...
    for row in items:
        ...
        inner = []
        if isinstance(row.get("list"), list) and row.get("list"):
            inner = row["list"]
        elif isinstance(row.get("items"), list) and row.get("items"):
            inner = row["items"]
        elif isinstance(row.get("subjects"), list) and row.get("subjects"):
            inner = row["subjects"]
        elif isinstance(row.get("movieList"), list) and row.get("movieList"):
            inner = row["movieList"]
        elif isinstance(row.get("customData"), dict) and isinstance(row["customData"].get("items"), list):
            inner = row["customData"]["items"]
        elif isinstance(row.get("banner"), dict) and isinstance(row["banner"].get("banners"), list):
...
def is_h264_compatible(stream_obj):
    ...
    return not any(k in codec or k in url for k in ["h265", "hevc", "x265", "hev1"])


# Ã¢â€â‚¬Ã¢â€â‚¬ Content endpoints Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
def get_detail(subject_id: str, depth: int = 0, session_id: Optional[str] = Cookie(None)):
    ...
    for f in ["isFavorite", "is_favorite", "fav", "is_fav", "collected", "isLike", "wantToSee", "likeStatus"]:
...
def get_episodes(series_id: str, session_id: Optional[str] = Cookie(None)):
    ...
    for s_raw in raw_seasons:
        ...
        for key in ["allEp", "epList", "episodeList", "episodes", "list", "items"]:
            ...
            if isinstance(pool, str):
                for e_num in pool.split(","):
                    if e_num:
                        eps.append({"episodeNumber": e_num, "title": f"Episode {e_num}"})
        ...
        if not eps:
            ...
            if isinstance(max_ep, str) and max_ep.isdigit():
                max_ep = int(max_ep)
...
# Ã¢â€â‚¬Ã¢â€â‚¬ BULLETPROOF STREAM RESOLVER (7 fallback tries) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
def get_stream(
    ...
    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 1: get_play_info Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    try:
        ...
        global_cookie = (
            res.get("signCookie") or data.get("signCookie") or
            s["client"].session.cookies.get("signCookie") or s["auth"].token or ""
        )
        all_subtitles = data.get("subTitleList") or []
        logger.info(f"[STREAM] try1 get_play_info: {len(raw_streams)} streams")
    except Exception as e:
        logger.error(f"[STREAM] try1 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 2: v_detail carrier=301 Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            v_res = s["client"].request(
                "POST", "/index/video/v_detail",
                data={"subjectId": subject_id, "carrier": "301", "quality": quality or "720p",
                      "episode": res_ep or 1, "season": res_se or 1},
            )
            ...
            if not all_subtitles:
                all_subtitles = v_data.get("subTitleList") or []
            logger.info(f"[STREAM] try2 v_detail 301: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try2 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 3: v_detail carrier=302 Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            v_res2 = s["client"].request(
                "POST", "/index/video/v_detail",
                data={"subjectId": subject_id, "carrier": "302", "quality": quality or "720p",
                      "episode": res_ep or 1, "season": res_se or 1},
            )
            v_data2 = v_res2.get("data") or {}
            raw_streams = v_data2.get("streamList") or v_data2.get("streams") or []
            if not global_cookie:
                global_cookie = v_data2.get("signCookie") or ""
            logger.info(f"[STREAM] try3 v_detail 302: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try3 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 4: play-compat Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            compat_res = s["client"].request(
                "GET", "/wefeed-mobile-bff/subject-api/play-compat",
                params={"subjectId": subject_id, "episode": res_ep or 1,
                        "season": res_se or 1, "resourceId": resource_id or ""},
            )
            compat_data = compat_res.get("data") or {}
            raw_streams = (
                compat_data.get("streamList") or compat_data.get("streams") or
                compat_data.get("resourceList") or []
            )
            if not global_cookie:
                global_cookie = compat_data.get("signCookie") or ""
            logger.info(f"[STREAM] try4 play-compat: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try4 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 5: resourceDetectors Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            det_res = s["client"].request(
                "GET", "/wefeed-mobile-bff/subject-api/get", params={"subjectId": subject_id}
            )
            detectors = (det_res.get("data") or {}).get("resourceDetectors") or []
            for det in detectors:
                if resource_id and str(det.get("resourceId")) != str(resource_id):
                    continue
                det_cookie = det.get("signCookie") or ""
                for res_item in (det.get("resolutionList") or []):
                    stream_url = res_item.get("resourceLink") or res_item.get("downloadUrl") or res_item.get("url")
                    if stream_url:
                        raw_streams.append({
                            "url": stream_url,
                            "quality": f"{res_item.get('resolution', 'Auto')}p",
                            "signCookie": res_item.get("signCookie") or det_cookie,
                            "id": res_item.get("resourceId") or det.get("resourceId") or "",
                            "codec": res_item.get("codecName") or "",
                        })
            logger.info(f"[STREAM] try5 resourceDetectors: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try5 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 6: resource-list Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            rlist_res = s["client"].request(
                "GET", "/wefeed-mobile-bff/subject-api/resource-list", params={"subjectId": subject_id}
            )
            for det in (rlist_res.get("data") or {}).get("resourceDetectors") or []:
                det_cookie = det.get("signCookie") or ""
                for res_item in (det.get("resolutionList") or []):
                    stream_url = res_item.get("resourceLink") or res_item.get("downloadUrl") or res_item.get("url")
                    if stream_url:
                        raw_streams.append({
                            ...
                            "quality": f"{res_item.get('resolution', 'Auto')}p",
                            "signCookie": res_item.get("signCookie") or det_cookie,
                            "id": res_item.get("resourceId") or "",
                            "codec": res_item.get("codecName") or "",
                        })
            logger.info(f"[STREAM] try6 resource-list: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try6 error: {e}")

    # Ã¢â€â‚¬Ã¢â€â‚¬ Try 7: fresh guest session Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
    if not raw_streams:
        try:
            fresh_auth = MovieBoxAuth()
            fresh_client = MovieBoxClient(auth=fresh_auth)
            fresh_content = MovieBoxContent(fresh_client)
            fresh_stream = MovieBoxStream(fresh_client)
            fresh_content.get_categories(category_id=1, page=1)  # bootstrap
            fresh_res = fresh_stream.get_play_info(subject_id, season=res_se, episode=res_ep)
            fresh_data = fresh_res.get("data") or {}
            raw_streams = fresh_data.get("streamList") or fresh_data.get("streams") or []
            if not global_cookie:
                global_cookie = fresh_data.get("signCookie") or fresh_auth.token or ""
            logger.info(f"[STREAM] try7 fresh session: {len(raw_streams)} streams")
        except Exception as e:
            logger.error(f"[STREAM] try7 error: {e}")

    if not raw_streams:
        logger.error(f"[STREAM] ALL 7 tries failed for subjectId={subject_id}")
        raise HTTPException(status_code=404, detail="No streams found after 7 attempts. Content may be region-locked or truly unavailable.")

    # H.264 prefer, then quality match
    ...
    proxy_url = f"/stream-proxy?u={quote(raw_url, safe='')}&c={quote(cookie, safe='')}"
...
# Ã¢â€â‚¬Ã¢â€â‚¬ UNIVERSAL M3U8 REWRITE & SEGMENT PROXY Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
async def stream_proxy(request: Request, u: str, c: Optional[str] = ""):
        ...
        try:
            async with httpx.AsyncClient(verify=False, timeout=20.0, follow_redirects=True) as client:
                ...
                if resp.status_code != 200:
                    raise HTTPException(status_code=resp.status_code, detail="CDN M3U8 error")
                ...
                for line in lines:
                    ...
                    if stripped.startswith("#"):
                        ...
                        rewritten.append(f"/stream-proxy?u={quote(full_url, safe='')}&c={quote(cookie_val, safe='')}")
    ...
    try:
        ...
        resp_headers = {"Access-Control-Allow-Origin": "*", "Access-Control-Allow-Headers": "*"}
...
async def proxy_download(
    ...
    s = get_session(session_id)
    c_res = s["client"].request("POST", "/index/video/v_detail",
                                data={"subjectId": subject_id, "carrier": "301", "quality": quality})
    data = c_res.get("data") or {}
    ...
    match = next((st for st in streams if ".mp4" in st.get("url", "").lower()), streams[0])
    url = match.get("url")
    ...
    if ".mp4" in url.lower():
        ...
        dl_client = httpx.AsyncClient(verify=False)
        req = dl_client.build_request("GET", url, headers=req_hdrs)
        r = await dl_client.send(req, stream=True)
        ...
        return StreamingResponse(stream_generator(), status_code=r.status_code, headers=headers, background=dl_client.aclose)
...
# Ã¢â€â‚¬Ã¢â€â‚¬ Subtitles Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
def get_subtitles(subject_id: str, se: int = 1, ep: int = 1, session_id: Optional[str] = Cookie(None)):
    ...
    try:
        ...
        try:
            det_res = s["client"].request("GET", "/wefeed-mobile-bff/subject-api/get",
                                          params={"subjectId": subject_id}, headers=hdrs)
...
def subtitle_search(query: str, session_id: Optional[str] = Cookie(None)):
    ...
    try:
        ...
        res = s["client"].request("GET", "/wefeed-mobile-bff/subject-api/subtitle-search",
                                  params={"q": query}, headers=hdrs)
        data = res.get("data", {})
        if not isinstance(data, dict):
            data = {}
...
# Ã¢â€â‚¬Ã¢â€â‚¬ History & Watchlist Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬

@app.get("/history/position")
def get_history_position(subject_id: str, resource_id: str, session_id: Optional[str] = Cookie(None)):
    s = get_session(session_id)
    return s["client"].request("GET", "/wefeed-mobile-bff/subject-api/resource-position",
...
def save_history_position(subject_id: str, resource_id: str, position: int, session_id: Optional[str] = Cookie(None)):
    s = get_session(session_id)
    return s["client"].request("POST", "/wefeed-mobile-bff/subject-api/resource-position",
                               data={"subjectId": subject_id, "resourceId": resource_id, "position": position})
...
def report_progress(req: ProgressReport, session_id: Optional[str] = Cookie(None)):
    ...
    if s["auth"].is_guest_mode:
        return {"status": "success"}
    return s["user"].report_history(req.subject_id, req.progress_ms, req.total_ms, status=req.status)


@app.post("/analytics/operation")
def track_operation(action: str, target: str, session_id: Optional[str] = Cookie(None)):
    s = get_session(session_id)
    return s["client"].request("POST", "/wefeed-mobile-bff/statistics/user-operation",
...
def get_history(page: int = 1, session_id: Optional[str] = Cookie(None)):
    ...
    combined = {str(x.get("subjectId") or x.get("id")): x for x in default_history + user_specific}
    user_history = list(combined.values())
    ...
    user_history = [
        ...
        if x.get("subjectId") and str(x.get("subjectId")) != "None" and str(x.get("subjectId")) not in blacklist
    ]
...
if __name__ == "__main__":
    uvicorn.run("moviebox_api_server:app", host="0.0.0.0", port=8000, reload=True)
